...
flowchart LR
    ...
    DEPLOY --> COMPUTE[Compute — EC2 / ECS]
...
```mermaid
flowchart LR
    subgraph Models["AWS Pricing Models"]
        OD[On-Demand<br/>Flexible, highest $/hr]
        RI[Reserved / Savings Plans<br/>Commitment, lower $/hr]
        SP[Spot<br/>Cheapest, interruptible]
        FT[Free Tier<br/>Limited, for learning]
    end
...
> Percentages vary by architecture. Use Cost Explorer for real data.

---

## 8. Migration Path (6 Rs — Preview)

```mermaid
flowchart LR
    A[On-Prem App] --> B{Strategy?}
    B -->|Rehost| C[Lift to EC2]
    B -->|Replatform| D[Move DB to RDS]
    B -->|Refactor| E[Containers / microservices]
    B -->|Retire| F[Shut down]
    B -->|Retain| G[Stay on-prem]
```

Full migration guidance appears in AWS CAF (Module 02).
...
- [AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) — official icons for your own diagrams

---

**Module complete.** Continue to [02-aws-foundation](../02-aws-foundation/README.md).
