...
| **Cloud** | IT infrastructure owned by a provider (e.g., AWS), accessed over the internet, billed on usage |
...
| **Scaling** | Buy more hardware (slow, expensive) | Auto Scaling, elastic resources |
| **Maintenance** | Your team patches OS, replaces disks, fixes hardware | AWS maintains physical layer; you manage OS and above (for IaaS) |
| **Geographic reach** | Build data centers in each region | Use existing AWS Regions worldwide |
...
```mermaid
flowchart LR
    A[Launch 2 instances] --> B[Traffic spikes]
    B --> C[Scale to 10 instances]
    C --> D[Traffic drops]
    D --> E[Scale back to 2]
    E --> F[Bill matches usage]
```

> **Important:** Cloud is not automatically cheaper. Forgotten resources, oversized instances, and always-on dev environments can make cloud *more* expensive than on-prem.
...
| Managed services | Let AWS run databases (RDS) instead of patching MySQL yourself |
...
|----------|--------------|--------------|
| **Lift and shift (rehost)** | Move the same VM/app to EC2 with minimal changes | Fast migration; may not optimize cost or scaling |
| **Replatform** | Small changes — e.g., move DB to RDS | Better managed services, moderate effort |
...
**Cloud:** EventBridge triggers a Lambda or starts an EC2 instance at 2 AM, job finishes, resource stops. Pay for 2 hours.
...
**Cloud:** Auto Scaling Group adds instances behind ALB for the sale weekend, scales down Monday.

### Scenario 3: Developer Test Environments

**On-prem:** IT provisions 5 VMs that developers "forget" to return.

**Cloud:** Terraform creates `dev-env-monday`, destroys `dev-env-friday` on a schedule. Tagged resources make cost visible.
...
|----------|------------|--------|
| Company email (standard) | SaaS (Google Workspace) | Not core business; provider specializes |
| Custom payment API | Cloud (ECS/EC2) | Needs scaling, CI/CD, global reach |
| 15-year-old mainframe billing | Retain on-prem (for now) | Rewrite cost is too high short-term |
| Student AWS lab | Cloud | Learn without buying hardware |
...
4. **What is lift and shift?**
   - *Moving applications to cloud with minimal architectural changes, often VM to EC2.*
...
- [AWS — 6 Strategies for Migration (6 Rs)](https://docs.aws.amazon.com/prescriptive-guidance/latest/migration-retiring-applications/welcome.html) — migration approaches
- [AWS Economics Center](https://aws.amazon.com/economics/) — TCO and cost perspectives
- [AWS Cloud Adoption Framework](https://aws.amazon.com/cloud-adoption-framework/) — organizational migration guidance (covered in Module 02)
